...
class TestTestGenerator:
    ...
    def test_generate_creates_test_file(
        ...
        assert result[0].name == "devices_tests.yaml"
    ...
    def test_generate_with_limit_to_groups(
        ...
        # DC1 IPs should be present, DC2 should not
        assert "192.168.1.10" in content  # spine01 DC1
        assert "192.168.1.20" in content  # leaf01 DC1
        assert "192.168.2.10" not in content  # dc2-spine01 DC2
    ...
    def test_generate_file_content(
        ...
        assert "---" in content  # YAML document start
        assert "ANTA Tests" in content
        assert "anta.tests.connectivity" in content
        assert "VerifyReachability" in content
        assert "Generated by avd-cli" in content
...
class TestGenerateAll:
    ...
    def test_generate_all_with_limit_to_groups(
        ...
        assert len(docs) == 2
        # Tests still generates 1 file, but filtered content
        assert len(tests) == 1
