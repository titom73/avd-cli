...
def update_pyproject_classifiers(pyproject_path: Path, versions: List[str]) -> None:
    ...
    # Handle both single and double quotes, and both 2 and 4 space indentation
    pattern = r'[ ]{2,4}["\']Programming Language :: Python :: \d+\.\d+["\'][,\n]+'
    content_cleaned = re.sub(pattern, "", content)

    # Now insert new classifiers after 'Programming Language :: Python :: 3'
    # Find the line with this classifier (try both quote styles)
    insertion_marker_single = "'Programming Language :: Python :: 3'"
    insertion_marker_double = '"Programming Language :: Python :: 3"'

    insertion_pos = content_cleaned.find(insertion_marker_double)
    use_double_quotes = True

    if insertion_pos == -1:
        insertion_pos = content_cleaned.find(insertion_marker_single)
